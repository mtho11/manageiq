...
function MwAddDatasourceService($http, $q) {
  ...
  self.findDsSelectionFromDriver = function(driverSelection) {
    ...
    if (self.isValidDatasourceName(driverSelection.id)) {
      ...
      var selectedDriverClass;
      if (driverSelection.driverClass !== undefined) {
        selectedDriverClass = driverSelection.driverClass;
      } else {
        selectedDriverClass = driverSelection.xaDsClass;
      }
      dsSelection = self.findDatasourceByDriverClass(selectedDriverClass);
    }
    return dsSelection;
  };
